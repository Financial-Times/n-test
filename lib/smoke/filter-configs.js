...
const askForSets = async (config) => {
	const possibilities = config.map((test, index)=> `${test.name || 'Untitled ' + index } (${Object.keys(test.urls).length} URLs)`);
	...
	return [].concat(responses.choices.map(response => response.replace(/ \(\d+ URLs\)/, '')));
};
