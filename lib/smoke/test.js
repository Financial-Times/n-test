		...
		Object.keys(options.urls).forEach(url => {
			...
			describe(url, () => {
				...
				if (test.check.performance) {
					const threshold = isNaN(test.check.performance) ? 2000 : test.check.performance;
					it(`should meet performance baselines - firstContentfulPaint should be < ${threshold}ms`, async () => {
						...
						expect(paints['first-paint']).toBeLessThan(threshold);
						expect(paints['first-contentful-paint']).toBeLessThan(threshold);
					});
