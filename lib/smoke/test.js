...
describe(`Smoke Tests for ${host}`, () => {
	...
	config.forEach((options) => {
		Object.keys(options.urls).forEach(url => {
			...
			const testOptions = Object.assign(urlOpts, { headers: options.headers}, {});
			const fullUrl = `${host}${url}`;
			let test = new URLTest(fullUrl, testOptions);
			...
			describe(url, () => {
				...
				if (test.check.status) {
					it(`should return ${test.check.status}`, () => {
						if(typeof test.check.status === 'string') {
							expect(test.response.url()).to.equal(host + test.check.status);
						} else {
