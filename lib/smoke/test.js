		...
		Object.keys(options.urls).forEach(url => {
			...
			describe(url, () => {
				...
				if (test.check.performance) {
					...
					it('should meet performance baselines - firstContentfulPaint should be < 2s', async () => {
						...
						expect(paints['first-paint']).toBeLessThan(2000);
						expect(paints['first-contentful-paint']).toBeLessThan(2000);
					});
